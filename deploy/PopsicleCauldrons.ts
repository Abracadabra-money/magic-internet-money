...
import { PopsiclePLPOracle } from "../typechain/PopsiclePLPOracle";
...
const ParametersPerChain = {
  [ChainId.Mainnet]: {
    ...
    cauldrons: [
      {
        proxyOracleName: "PopsicleUSDCWETHProxyOracle",
        oracleName: "PopsicleUSDCWETHOracle",
        ...
        token1Aggregator: "0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419", // ETH
      },
    ],
  },
};
...
const deployFunction: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  for (let i = 0; i < parameters.cauldrons.length; i++) {
    ...
    // Proxy Oracle
    await deploy(parameter.proxyOracleName, {
      from: deployer,
      args: [],
      log: true,
    ...
    await deploy(parameter.oracleName, {
      from: deployer,
    ...
    const ProxyOracle = await ethers.getContract<ProxyOracle>(parameter.proxyOracleName);
    ...
    const PopsiclePLPOracle = await ethers.getContract<PopsiclePLPOracle>(parameter.oracleName);
