...
contract AvaxUsdtSwapper is ISwapperGeneric {
    ...
    function swap(
        ...
        (uint256 usdcAmount, uint256 avaxAmount) = AVAXUSDT.burn(address(this));
        
        // swap USDC to AVAX
        (uint256 reserve0, uint256 reserve1, ) = AVAXUSDT.getReserves();
        uint256 avaxFromUsdc = _getAmountOut(usdcAmount, reserve0, reserve1);
        USDC.transfer(address(AVAXUSDT), usdcAmount);
        AVAXUSDT.swap(0, avaxFromUsdc, address(this), new bytes(0));
