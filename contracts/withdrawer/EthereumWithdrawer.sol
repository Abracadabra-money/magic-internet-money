...
contract EthereumWithdrawer is BoringOwnable {
    ...
    AnyswapRouter public constant ANYSWAP_ROUTER = AnyswapRouter(0x765277EebeCA2e31912C9946eAe1021199B39C61);
    ...
    constructor(
        Cauldron[] memory pools,
        CauldronV1[] memory poolsv1,
        Cauldron[] memory degens_
    ) {
        bentoBoxCauldronsV2 = pools;
        bentoBoxCauldronsV1 = poolsv1;
        degenBoxCauldrons = degens_;
    ...
    function withdraw() public {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (feesEarned > (BENTOBOX.toAmount(MIM, BENTOBOX.balanceOf(MIM, address(bentoBoxCauldronsV2[i])), false))) {
                MIM.transferFrom(MULTISIG, address(BENTOBOX), feesEarned);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (feesEarned > (DEGENBOX.toAmount(MIM, DEGENBOX.balanceOf(MIM, address(degenBoxCauldrons[i])), false))) {
                MIM.transferFrom(MULTISIG, address(DEGENBOX), feesEarned);
    ...
    // support slippage to avoid frontrunning
    ...
    function bridgeSpell(
        uint256 amount,
        address to,
        uint256 toChain
    ) external onlyVerified {
        ANYSWAP_ROUTER.anySwapOutUnderlying(SPELL, to, amount, toChain);
    }

    function setAllFeeTo(address feeTo) public onlyOwner {
        uint256 length = bentoBoxCauldronsV2.length;
        for (uint256 i = 0; i < length; i++) {
            if (bentoBoxCauldronsV2[i].feeTo() != feeTo) {
                bentoBoxCauldronsV2[i].setFeeTo(feeTo);
            }
        }

        length = bentoBoxCauldronsV1.length;
        for (uint256 i = 0; i < length; i++) {
            if (bentoBoxCauldronsV1[i].feeTo() != feeTo) {
                bentoBoxCauldronsV1[i].setFeeTo(feeTo);
            }
        }

        length = degenBoxCauldrons.length;
        for (uint256 i = 0; i < length; i++) {
            if (degenBoxCauldrons[i].feeTo() != feeTo) {
                degenBoxCauldrons[i].setFeeTo(feeTo);
            }
        }
    }

    /// @notice updating all pools so that withdrawFees deposit the fees
    /// to each respective bentbox implementation for this contract
    function setAllFeeToThis() external onlyOwner {
        setAllFeeTo(address(this));
    }
    ...
    function _addPool(Cauldron pool) internal onlyOwner {
        //allow settings and changing bentoBoxCauldronsV2 that receive staking rewards.
