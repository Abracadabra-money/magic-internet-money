...
interface IPartialDeployParams {
  ...
  LIQUIDATION_SEIZE_COLLATERAL?: boolean;
}
...
describe("Private Lending Pool", async () => {
  ...
  const showBalances = async (accs = { alice, bob, carol }) => {
    for (const [name, acc] of Object.entries(accs)) {
      ...
      async function show(token: string, contract: WETH9Mock | ERC20Mock) {
      ...
      await show("WETH", weth);
      await show("Guineas", guineas);
  ...
  before(async () => {
    ...
    masterContract = await deployContract("PrivatePool", bentoBox.address);
    ...
    oracle = await deployContract("OracleMock");
  ...
  describeSnapshot("Repay", () => {
    ...
    it("Should use repayment towards fees owed, if any", async () => {
      ...
      const t0 = {
        ...
        bobDebtPart: bobLoanAmount.mul(1001).div(1000),
      ...
      const t1AccruedInterest = t0.bobDebtAmount.mul(MainTestSettings.INTEREST_PER_SECOND.mul(timeStep)).div(one);
      ...
      const carolLoanShare = t0.assetBalance.reservesShare.sub(t1AccruedInterest.div(10).mul(9).div(20)).add(1).mul(10_000).div(10_001);
      ...
      const t3AccruedInterest = t2.totalDebt.elastic.mul(MainTestSettings.INTEREST_PER_SECOND.mul(timeStep)).div(one);
      // Intermediate value; we would see it if we did something that accrues
      // but not deposits any assets:
      const t3FeesOwedBeforeRepay = t2.feesOwedAmount.add(t3AccruedInterest.div(10));
      ...
      const repayPart = t3FeesOwedBeforeRepay;
      ...
      expect(t3.assetBalance.feesEarnedShare).to.equal(t2.assetBalance.feesEarnedShare.add(t3FeesOwedBeforeRepay.mul(9).div(20)));
      ...
      // to asset reserves.
      const excessRepayAmount = repayPart.mul(t3.totalDebt.elastic).div(t3.totalDebt.base).sub(t3FeesOwedBeforeRepay);
  ...
  describeSnapshot("Liquidate -- normal", () => {
    ...
    const getState = async () => {
      ...
      return {
        aliceBentoGuineas: await bentoBox.balanceOf(guineas.address, a),
        aliceBentoWeth: await bentoBox.balanceOf(weth.address, a),

        bobCollateralShare: await mainPair.userCollateralShare(b),
        carolCollateralShare: await mainPair.userCollateralShare(c),

        bobDebtPart: await mainPair.borrowerDebtPart(b),
        carolDebtPart: await mainPair.borrowerDebtPart(c),
  ...
  describeSnapshot("Liquidate -- in kind", () => {
    ...
    const getState = async () => {
      ...
      return {
        ...
        bobCollateralShare: await pair.userCollateralShare(b),
        carolCollateralShare: await pair.userCollateralShare(c),

        bobDebtPart: await pair.borrowerDebtPart(b),
        carolDebtPart: await pair.borrowerDebtPart(c),
