...
const deployFunction: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const collateral = "0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83"; //xBOO
  const oracle = await ethers.getContract<ProxyOracle>("FTMDegenProxyOracle");
  ...
  const cauldronAddress = await getDeployment("ftmDegen")
...
deployFunction.tags = ["ftmDegenCauldron"];
deployFunction.dependencies = ["ftmOracle"];
