...
const deployFunction: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const collateral = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"; // wETH
  const oracle = await ethers.getContract<ProxyOracle>("WethProxyOracle");
  ...
  const cauldronAddress = await getDeployment("WethCauldron")
...
deployFunction.dependencies = ["WethOracle"];
