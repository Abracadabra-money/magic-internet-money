...
describe("MagicCRV", async () => {
  ...
  it.only("should allow arbitrary execution only when flag enabled", async () => {
    ...
    await expect(CurveVoter.execute(CRV.address, 0, data)).to.be.revertedWith("NotMigrating()");

    await CurveVoter.setMigrationEnabled(true);
