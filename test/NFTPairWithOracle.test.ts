...
describe("NFT Pair With Oracle", async () => {
  ...
  describeSnapshot("Repay", () => {
    ...
    it("Should allow partial repayments", async () => {
      ...
      expect(await apes.ownerOf(apeIds.aliceOne)).to.equal(pair.address);
      expect((await pair.tokenLoanParams(apeIds.aliceOne)).valuation).to.equal(afterParams.valuation);
      ...
      const paid = t0.alice.sub(t1.alice);
      expect(paid).to.be.gte(partValuationShare.add(linearInterest));
      expect(paid).to.be.lte(maxRepayShare);
      ...
      const fee = t1.feeTracker.sub(t0.feeTracker);
      expect(fee.mul(10)).to.be.gte(linearInterest);
      expect(fee.mul(10)).to.be.lte(paid.sub(partValuationShare));
      expect(t1.pair.sub(t0.pair)).to.equal(fee);
  ...
  describeSnapshot("Signed Lend/Borrow", () => {
    ...
    describe("Borrow", () => {
      ...
      it("Should allow signature reuse unless revoked by signer", async () => {
        ...
        // It succeeds again, because the signature has not been revoked:
        await expect(pair.connect(alice).takeCollateralAndLend(apeIds.bobTwo, bob.address, loanParams, false, sigParams)).to.emit(
          pair,
          "LogLend"
        );
      ...
      it("Should allow signers to revoke signatures", async () => {
        ...
        // Bob revokes all his signatures
        await pair.connect(bob).incrementBatchId();

        // It fails, because the signature has been revoked:
        await expect(pair.connect(alice).takeCollateralAndLend(apeIds.bobTwo, bob.address, loanParams, false, sigParams)).to.be.revertedWith(
          "NFTPair: signature invalid"
        );
      });
